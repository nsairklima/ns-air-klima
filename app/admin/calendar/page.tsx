...
export default function CalendarPage() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      console.error("Fetch hiba (events):", e); 
    }
  };
  ...
  const fetchUnits = async () => {
    try {
      ...
      console.error("Fetch hiba (units):", e);
    }
  };
  ...
  const handleSave = async () => {
    // Validáció: Szerkesztésnél nem kötelező a unitId, új bejegyzésnél igen
    if (!editingId && !newEntry.unitId) {
      return alert("Kérlek, válassz ki egy ügyfelet!");
    }

    if (!newEntry.date) {
      return alert("Kérlek, adj meg egy időpontot!");
    }
    
    const parsedUnitId = parseInt(newEntry.unitId, 10);
    if (!editingId && isNaN(parsedUnitId)) {
      return alert("Érvénytelen ügyfél azonosító!");
    }

    const payload = {
      ...(editingId ? { id: editingId } : { unitId: parsedUnitId }),
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Hiba a mentés során: ${errData.message || res.statusText}`);
      }
    } catch (e) {
  ...
  const openEdit = (e: React.MouseEvent, eventData: any) => {
    ...
    const cleanId = typeof eventData.id === 'string' ? parseInt(eventData.id.replace('log-', ''), 10) : eventData.id;
    setEditingId(cleanId);
    setActiveType(eventData.type || "MAINTENANCE");
    setNewEntry({
      unitId: eventData.unitId?.toString() || eventData.unit?.id?.toString() || "",
  ...
  return (
    <div style={pageStyle}>
      <style>{`
        .type-btn { flex: 1; border: 2px solid transparent; color: #fff; padding: 10px; border-radius: 8px; cursor: pointer; font-size: 11px; opacity: 0.5; transition: 0.2s; }
                ...
                >
                  ...
                  {units.map((u: any) => (
                    <option key={u.id} value={u.id.toString()}>
                      {u.displayName || `${u.client?.name || 'Ismeretlen'} (${u.brand} ${u.model})`}
                    </option>
                  ))}
                </select>
                {!editingId && (
