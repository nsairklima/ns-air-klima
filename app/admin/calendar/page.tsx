...
export default function CalendarPage() {
  ...
  return (
    <div style={pageStyle}>
      <style>{`
        .calendar-content {
          transition: ${isTransitioning ? 'transform 0.15s ease-out, opacity 0.15s' : 'none'};
          ...
          {selectedDate ? (
            <div style={dailyContainer}>
              ...
              {dailyEvents.length === 0 ? (
                <p style={{ textAlign: 'center', color: '#94a3b8', marginTop: '40px' }}>Nincs feladat.</p>
              ) : (
                dailyEvents.map(ev => (
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div key={day} onClick={() => setSelectedDate(dateStr)} className="day-cell" style={cellStyle}>
                    <span style={{ fontSize: '13px', color: '#94a3b8' }}>{day}</span>
      ...
      {showModal && (
        <div style={modalOverlay}>
          <div style={modalContent}>
            <h3 style={{marginTop: 0}}>Bejegyzés</h3>
