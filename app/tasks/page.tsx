...
export default function TasksPage() {
  ...
  // Szerkesztett adatok elmentése (PUT kérés)
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Bezárjuk a modált és frissítjük a listát
      setEditingTask(null);
      await fetchTasks();
    } catch (err: any) {
    ...
    <div style={{ padding: "24px", maxWidth: "1000px", margin: "0 auto", color: "#ffffff" }}>
      ...
      {!loading && tasks.length > 0 && (
        <div style={{ display: "grid", gap: "16px", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
          {tasks.map((task) => (
            <div key={task.id} style={{ border: "1px solid #334155", padding: "16px", borderRadius: "8px", backgroundColor: "#1e293b", color: "#ffffff", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              <div>
            ...
            <form onSubmit={handleSave} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px", marginTop: "12px" }}>
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  style={{ padding: "8px 16px", backgroundColor: "#16a34a", color: "#ffffff", border: "none", borderRadius: "6px", cursor: "pointer" }}
                >
