...
function formatDate(dateString?: string) {
  ...
  try {
    const normalizedValue = dateString.replace("T", " ").slice(0, 19);
...
function createSearchVariants(address: string) {
  ...
  if (firstSpaceIndex === -1) {
    return [cleanAddress, cleanAddress + ", Magyarország"];
  }
  ...
  return Array.from(
    new Set([
      ...
      street + ", " + city,
      street + ", " + city + ", Magyarország",
      city + ", Magyarország",
    ])
  );
}

function getSavedCoordinates(task: Task): Coordinates | null {
  const latitude =
...
async function findCoordinates(address: string): Promise<Coordinates | null> {
  const normalizedAddress = normalizeAddress(address);

  if (!normalizedAddress) return null;

  const cacheKey = "tasks-map-coordinate-" + normalizedAddress;
  const cachedValue = window.localStorage.getItem(cacheKey);

  if (cachedValue) {
    try {
      const cachedCoordinates = JSON.parse(cachedValue) as Coordinates;
  ...
  for (let index = 0; index < searchVariants.length; index += 1) {
    const searchAddress = searchVariants[index];
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Nominatim HTTP hiba:", response.status, searchAddress);
      ...
      if (!contentType.includes("application/json")) {
        console.error("A címkereső nem JSON választ adott:", searchAddress);
        return null;
      }
      ...
      console.error("Címkeresési hiba:", searchAddress, error);
    }
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      ...
      const tasksWithLocation = tasks.filter((task) =>
        Boolean(getSavedCoordinates(task) || task.address?.trim())
      );
      ...
      for (let index = 0; index < tasksWithLocation.length; index += 1) {
        if (cancelled) return;

        const task = tasksWithLocation[index];
        setLoadingText(
          "Térkép feldolgozása: " +
            (index + 1) +
            " / " +
        ...
        let coordinates = getSavedCoordinates(task);

        if (!coordinates && task.address?.trim()) {
          if (fallbackSearches > 0) await wait(1100);
          if (cancelled) return;

          coordinates = await findCoordinates(task.address);
          fallbackSearches += 1;
        }
        ...
        const taskStatus = task.completed_at ? "✅ Kész" : "⏳ Folyamatban";
        ...
        const mapsQuery = task.address?.trim()
          ? task.address
        ...
        const infoContent =
          ...
          '<div style="font-size:16px;font-weight:bold;border-bottom:1px solid #ddd;padding-bottom:7px;margin-bottom:8px;">' +
          taskType +
          "</div>" +
          ...
          "</p>" +
          '<a href="' +
          escapeHtml(googleMapsSearchUrl) +
          '" target="_blank" rel="noopener noreferrer">' +
          ...
          task.id +
          '" style="width:100%;padding:8px;background:#2980b9;color:white;border:none;border-radius:6px;cursor:pointer;font-weight:bold;">' +
          "📋 Ugrás a feladathoz" +
          "</button>" +
          "</div>";
      ...
      if (successfulMarkers === 0) {
        setLoadingText("Egyik címhez sem található koordináta.");
    ...
    const loadGoogleMaps = () => {
      ...
      script.async = true;
      script.defer = true;
      script.onload = () => void initializeMap();
      script.onerror = () => {
        setLoadingText("A Google Maps nem tölthető be.");
      };
      ...
      >
        ...
        {notFoundAddresses.length > 0 && (
          <button
            type="button"
            onClick={() =>
              setShowNotFoundAddresses((currentValue) => !currentValue)
            }
            style={{
              background: "#fef5e7",
              color: "#9c640c",
          ...
          >
            ❌ Nem található: {notFoundAddresses.length}{" "}
            {showNotFoundAddresses ? "▲" : "▼"}
          </button>
        )}
      </div>

      {notFoundAddresses.length > 0 && showNotFoundAddresses && (
        <div
          style={{
            width: "100%",
        ...
        >
          ...
          >
            Az alábbi címeket nem sikerült megtalálni:
          </div>
          ...
          >
            {notFoundAddresses.map((item) => (
              ...
              >
                ...
                <div>📍 {item.address || "Nincs megadott cím"}</div>
              </div>
            ))}
          </div>
        </div>
